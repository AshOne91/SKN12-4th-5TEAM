...
async def chatbot_rooms(request: ChatbotRoomsRequest, session=Depends(require_session)):
    ...
    if chatbot_template is None:
        raise RuntimeError("ChatbotTemplateImpl is not registered in TemplateContext")
    return chatbot_template.on_chatbot_rooms_req(session, request)

@router.post("/room/new", response_model=ChatbotRoomNewResponse)
async def chatbot_room_new(request: ChatbotRoomNewRequest, session=Depends(require_session)):
    if isinstance(session, BaseResponse):
        return session
    chatbot_template = TemplateContext.get_template(TemplateType.CHATBOT)
    if chatbot_template is None:
        raise RuntimeError("ChatbotTemplateImpl is not registered in TemplateContext")
    return chatbot_template.on_chatbot_room_new_req(session, request)

@router.post("/message", response_model=ChatbotMessageResponse)
async def chatbot_message(request: ChatbotMessageRequest, session=Depends(require_session)):
    if isinstance(session, BaseResponse):
        return session
    chatbot_template = TemplateContext.get_template(TemplateType.CHATBOT)
    if chatbot_template is None:
        raise RuntimeError("ChatbotTemplateImpl is not registered in TemplateContext")
    return chatbot_template.on_chatbot_message_req(session, request)

@router.post("/history", response_model=ChatbotHistoryResponse)
async def chatbot_history(request: ChatbotHistoryRequest, session=Depends(require_session)):
    if isinstance(session, BaseResponse):
        return session
    chatbot_template = TemplateContext.get_template(TemplateType.CHATBOT)
    if chatbot_template is None:
        raise RuntimeError("ChatbotTemplateImpl is not registered in TemplateContext")
    return chatbot_template.on_chatbot_history_req(session, request) 